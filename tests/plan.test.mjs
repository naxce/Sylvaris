...
test("reminderItems lists the next two weeks of reminders for upload", async () => {
    const { reminderItems } = await import("../shell/lib/plan.mjs")
    const data = sample()
    data[0].groups[0].subs[0].dives.push({ id: "t2", text: "**Dentist**\nnotes", due: "2026-09-25", time: "10:00", remind: [30], alarm: true })
    data[0].groups[0].subs[0].dives.push({ id: "t3", text: "far", due: "2026-12-25", time: "10:00" })
    const items = reminderItems(data, NOW)
    assert.equal(items.length, 1)
    assert.deepEqual(Object.keys(items[0]).sort(), ["alarm", "at", "rid", "title"])
    assert.equal(items[0].title, "Dentist")
