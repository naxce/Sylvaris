...
The workspace you are on is a capsule with its number and the icons of the apps on it; other workspaces show their number, bright when something is open there, with a tick under it for each window (up to three), and a red ring when one of them asks for attention. App icons and ticks need Hyprland or niri. `bar.workspaceIcons` adds a glyph to the current capsule or turns the others into dots (`numbers`, `dots`, `paw`, `bone`, `heart`, `star`, `leaf`, `flower`, `fire`, `diamond`, `tree`, `ghost`, `moon`, `cat`, `fish`, `music`). The default, `auto`, uses the theme's `workspaceIcon` and numbers when it has none. Panels such as SylCenter and SylClock open next to the bar wherever it sits, so a bar on the left opens them along the left edge. On a computer without a battery the battery module hides itself and the rest close the gap. `floating: false` makes the bar span the edge; `enabled: false` turns it off.
